                               ...
                               std::size_t all_data_byte_size )
{
  ...
  std::fill( array.data.get() + last_chunk_byte_size + 1,
             array.data.get() + array.length, 0 );

  if ( isLittleEndian() ) {
    std::copy( reinterpret_cast<char *>( &bit_count ),
               reinterpret_cast<char *>( &bit_count ) + 8,
               std::reverse_iterator( array.data.get() + array.length ) );
  } else {
    *reinterpret_cast<uint64_t *>( array.data.get() + array.length - 8 ) =
        bit_count;
  }
...
uint32_t f( std::size_t index, uint32_t B, uint32_t C, uint32_t D )
{
  ...
  throw std::invalid_argument( "f function invalid index: " +
                               std::to_string( index ) );
}
...
uint32_t get_K_constant( std::size_t index )
{
  ...
  throw std::invalid_argument( "K constant function invalid index: " +
                               std::to_string( index ) );
}
...
byte_array encrypter::encrypt( std::istream &stream ) const
{
  ...
  auto last_chunk =
      process_last_chunk( chunk, last_chunk_size, byte_processed );

  for ( int i = 0; i < last_chunk.length * 8 / CHUNK_BIT_SIZE; ++i ) {
    auto temp_base =
        encrypt_chunk( last_chunk.data.get() + i * CHUNK_BIT_SIZE / 8, H );
    for ( std::size_t i = 0; i < H.size(); ++i ) {
      H[ i ] += temp_base[ i ];
    }
  }
  ...
  if ( isLittleEndian() ) {
    for ( int i = 0; i < H.size(); ++i ) {
      std::copy(
          reinterpret_cast<char *>( H.data() ) + i * 4,
          reinterpret_cast<char *>( H.data() ) + i * 4 + 4,
          std::reverse_iterator( encrypted_data.data.get() + i * 4 + 4 ) );
    }
  } else {
    std::copy( reinterpret_cast<char *>( H.data() ),
               reinterpret_cast<char *>( H.data() ) + 20,
...
std::array<uint32_t, 5> encrypter::encrypt_chunk(
    const char *chunk, const std::array<uint32_t, 5> &H ) const
{
  ...
  for ( int i = 0; i < bigger_chunk.size(); ++i ) {
    if ( i < 16 ) {
      if ( isLittleEndian() ) {
        std::copy( &chunk[ i * 4 ], &chunk[ i * 4 ] + 4,
                   std::reverse_iterator(
                       reinterpret_cast<char *>( &bigger_chunk[ i ] ) + 4 ) );
      } else {
        bigger_chunk[ i ] =
            *reinterpret_cast<const uint32_t *>( chunk + i * 4 );
      }
    } else {
      bigger_chunk[ i ] = circular_left_shift(
          ( bigger_chunk[ i - 3 ] ^ bigger_chunk[ i - 8 ] ^
            bigger_chunk[ i - 14 ] ^ bigger_chunk[ i - 16 ] ),
          1 );
    }

    uint32_t temp = circular_left_shift( a, 5 ) + f( i, b, c, d ) + e +
                    bigger_chunk[ i ] + get_K_constant( i );
